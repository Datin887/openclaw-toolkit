# 🛠️ OpenClaw Toolkit

> Модульный набор навыков, скриптов и конфигураций для [OpenClaw](https://github.com/openclaw/openclaw) — AI-агент платформы.

**[English](README.md)**

## 📦 Содержимое

### Навыки (Skills)

| Навык | Описание | Установка |
|-------|----------|-----------|
| **browser-cdp** | Headless браузер через CDP (Chrome DevTools Protocol). Навигация, скриншоты, JS-выполнение, клики, заполнение форм. Работает без GUI — идеально для серверов. | [Подробнее ↓](#browser-cdp) |
| **fire_copywriting_engine** | Огненные продающие тексты по проверенным техникам директ-риспонса. Рерайт маркетинговых текстов, лендингов, писем, рекламы. На основе Ogilvy, Sugarman, Settle, Kennedy, Carlton и других. | [Подробнее ↓](#fire_copywriting_engine) |
| **deep_audience_analysis_premium** | Глубокий психологический анализ целевой аудитории. Профиль клиента, боли, страхи, желания, JTBD, возражения, трансформация, маркетинговые углы. 21 секция анализа. | [Подробнее ↓](#deep_audience_analysis_premium) |

### Темы (Themes)

| Тема | Описание | Установка |
|------|----------|-----------|
| **Laguna** | Тёплый кремовый + фиолетовый (светлая) / глубокий чёрный + голубой (тёмная). Шрифт Inter, бруталистские тени. С [tweakcn.com](https://tweakcn.com/themes/cmpc347ze000204l580xtaqet). | [INSTALL.md](themes/laguna/INSTALL.md) |

---

## 🚀 Быстрая установка

### Вариант 1: Через агента (рекомендуется)

Попроси своего OpenClaw агента:

> «Клонируй `https://github.com/Datin887/openclaw-toolkit.git`, посмотри список навыков и установи нужные»

Агент сам разберётся что к чему.

### Вариант 2: Вручную

```bash
# 1. Клонируй репозиторий
git clone https://github.com/Datin887/openclaw-toolkit.git

# 2. Скопируй нужный навык в plugin-skills
cp -r openclaw-toolkit/skills/browser-cdp ~/.openclaw/plugin-skills/

# 3. Зарегистрируй навык в конфиге OpenClaw
# Добавь в skills.entries и skills.load.extraDirs (см. документацию навыка)

# 4. Перезапусти gateway
openclaw gateway restart
```

---

## 📋 Навыки

### browser-cdp

**Headless browser automation через Chrome DevTools Protocol.**

**Что умеет:**
- 🌐 Навигация по URL
- 📸 Скриншоты страниц
- ⚡ Выполнение JavaScript
- 🖱️ Клики по элементам
- ✏️ Заполнение форм
- 📄 Извлечение контента страницы
- 🔄 Управление вкладками

**Требования:**
- Chromium headless как systemd сервис (порт 9222)
- Python пакет: `websocket-client`

**Быстрый старт:**
```bash
# Запуск браузера
sudo systemctl start chromium-headless

# Навигация
python3 scripts/browser_cdp.py navigate "https://example.com"

# Скриншот
python3 scripts/browser_cdp.py screenshot /tmp/screenshot.png

# JS-выполнение
python3 scripts/browser_cdp.py evaluate "document.title"
```

**Полная документация:** [skills/browser-cdp/SKILL.md](skills/browser-cdp/SKILL.md)

### fire_copywriting_engine

**Огненные продающие тексты по техникам директ-риспонса.**

**Что умеет:**
- 🔥 Превращает сухие тексты в убедительные продающие
- 🧠 Психологическое сканирование — определяет боли, страхи, желания ЦА
- 📐 Структура AIDA — Внимание, Интерес, Желание, Действие
- 💬 Разговорный стиль — как друг, а не корпорация
- 🎬 Кинематографичный сторителлинг — показывай, а не рассказывай
- 📊 Фактологический фундамент — подтверждай обещания данными
- ⚡ Ударный CTA — бескомпромиссный призыв к действию

**Алгоритмы:** Sugarman/Bencivenga, Ogilvy, Ben Settle, Haddad/Godin, Kennedy/Bly, Carlton, Makepeace

**Быстрый старт:**
```
Пришли исходный текст и попроси переписать как огненный продающий.
Или триггерь: "огненный текст", "продающий текст", "копирайтинг"
```

**Полная документация:** [skills/fire_copywriting_engine/SKILL.md](skills/fire_copywriting_engine/SKILL.md)

### deep_audience_analysis_premium

**Глубокий психологический анализ целевой аудитории — 21 секция.**

**Что умеет:**
- 🧠 Глубинная психология клиента — боли, страхи, желания, стыд, вина, зависть
- 📐 21-секционный фреймворк — от профиля до маркетинговых выводов
- 💬 Язык клиента — аутентичные цитаты и внутренние монологи
- 🎯 JTBD-анализ — функциональная, эмоциональная, социальная, скрытая задачи
- 🚧 Работа с возражениями — что блокирует покупку и почему
- ✨ Карта трансформации — путь клиента «до» и «после»

**Секции вывода:** Резюме, Профиль, Психографика, Основная проблема, JTBD, Топ-5 эмоций, Топ-5 страхов, Влияние на отношения, Болезненные фразы, Прошлые решения, Точки сопротивления, Идеальная трансформация, Пост-трансформационные цитаты, Мифы и заблуждения, Кого винит, Где застрял, Что отпустить, Топ-5 возражений, Триггеры покупки, Маркетинговые выводы, Гипотезы.

**Быстрый старт:**
```
Предоставь информацию о продукте, рынке, сегментах ЦА, конкурентах, позиционировании, каналах продаж и цели анализа.
Или просто опиши продукт и скажи: "Сделай глубокий анализ ЦА"
```

**Полная документация:** [skills/deep_audience_analysis_premium/SKILL.md](skills/deep_audience_analysis_premium/SKILL.md)

---

## 🎨 Темы

### Laguna

Красивая тема с [tweakcn.com](https://tweakcn.com/themes/cmpc347ze000204l580xtaqet).

| Режим | Описание |
|-------|----------|
| Светлая | Тёплый кремовый фон, фиолетовый акцент, шрифт Inter |
| Тёмная | Глубокий чёрный фон, голубой акцент, бруталистские тени |

См. [INSTALL.md](themes/laguna/INSTALL.md).

---

## 📁 Структура

```
openclaw-toolkit/
├── README.md              ← English version
├── README.ru.md           ← Ты здесь
├── LICENSE
├── skills/                ← Навыки для OpenClaw
│   ├── browser-cdp/
│   │   ├── SKILL.md
│   │   └── scripts/
│   │       └── browser_cdp.py
│   ├── nemotron-vision/
│   │   ├── SKILL.md
│   │   └── scripts/
│   │       └── nemotron_vision.py
│   ├── fire_copywriting_engine/
│   │   ├── SKILL.md
│   │   └── references/
│   │       └── system_prompt.md
│   └── deep_audience_analysis_premium/
│       ├── SKILL.md
│       └── references/
│           └── system_prompt.md
├── themes/                ← Темы для Control UI
│   └── laguna/
│       ├── theme.json
│       ├── theme.css
│       └── INSTALL.md
├── scripts/               ← Вспомогательные скрипты
├── configs/               ← Шаблоны конфигураций
└── docs/                  ← Документация
```

## 📜 Лицензия

MIT — делай что хочешь. Подробности в [LICENSE](LICENSE).
